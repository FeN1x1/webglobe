              ...
              <tr v-for="person in persons" :key="person.id">
                ...
                <td class="px-2 py-4 text-sm font-medium text-right whitespace-nowrap">
                  <router-link
                    ...
                    :to="{ name: 'editPerson', params: { id: person.id, currentPage: page } }"
                    >Upraviť</router-link
                  >
                </td>
                <td class="py-4 pr-6 text-sm font-medium text-right whitespace-nowrap">
                  <span @click="deletePerson(person.id)" class="text-indigo-600 cursor-pointer hover:text-indigo-900"
                    >Vymazať</span
                  >
                </td>
              </tr>
        ...
        <div class="flex justify-center p-3 px-6">
          <div class="items-center mt-6 space-y-2 text-sm font-medium text-gray-500 sm:space-y-0 sm:space-x-3 sm:flex">
            ...
            <div class="space-x-1">
              <router-link
                v-if="page != 1"
                :to="{ name: 'personsList', query: { page: page - 1 } }"
...
export default {
  computed: {
    ...mapState({
      persons: (state) => state.person.pagination.paginated,
    }),
    ...mapState("person", ["count", "perPage", "pages", "pagination", "filter"]),
    page() {
    ...
    fetchData() {
      this.$store.dispatch("person/fetchList", this.page);
    },
  },
  components: {
    Header,
  ...
  methods: {
    ...mapActions("person", ["delete", "addFilter", "removeFilter"]),
    deletePerson(id) {
    ...
    setFilter(filterType) {
      if (filterType) {
        this.$store.dispatch("person/addFilter", {
          name: this.$refs.filter.value,
        });
      } else {
        this.$store.dispatch("person/removeFilter");
      }
      this.fetchData;
    },
  },
